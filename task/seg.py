...
def extract_patches(image: np.ndarray, patch_size: Tuple[int, int], overlap: int) -> Tuple[
    List[np.ndarray], List[Tuple[int, int, int, int]]]:
    """
    将图像分割成重叠的patches
...
def merge_predictions(predictions: List[np.ndarray], coords: List[Tuple[int, int, int, int]],
...
def predict_single_image(model: torch.nn.Module, image_path: str, device: torch.device,
    ...
    if output_path:
        try:
            # 创建三通道输出图像
            output_image = np.zeros((*original_size, 3), dtype=np.uint8)
            # 在B通道设置翼状胬肉区域为128
            output_image[..., 2] = (final_prediction > 0.5).astype(np.uint8) * 128
            ...
            plt.subplot(133)
            overlay = image.copy()
            overlay[final_prediction > 0.5] = [0, 0, 1]  # 蓝色标记分割区域
            plt.imshow(overlay)
...
def main():
    ...
    # 加载模型
    model_path = "best_model_dice_0.9001_acc_0.9001.pth"  # 请替换为实际的模型路径
    model = load_model(model_path, device)

    # 设置输入输出路径
    input_dir = "val_img"  # 请替换为实际的图像目录
